...
There are two AWS accounts. Both have VPCs connected via **Transit Gateway** (Zencity is peering-free by policy).
...
```
        ...
        └────────────────────────────────────────────────┘
                              ...
                              (TGW)
                                 │
        ┌────────────────────────────────────────────────┐
        │  AWS account B — "workload" (e.g. engage-staging)│
        │                                                  │
        │   EKS cluster                                    │
        │      │                                           │
        │      ▼                                           │
...
This pattern was **end-to-end validated on `lab-staging` → shared-edge** on May 12, 2026. The next day (May 13), it rolled to all six staging clusters (`lab`, `shared`, `platform`, `engage`, `organic`, `survey`) — Pod Identity association, cross-account IAM role, TargetGroupBinding CRDs, SG egress rules, readiness gates — using the ArgoCD ApplicationSet pattern from [GitOps engine](/architecture/gitops-engine/) to fan out the cluster-side config.
