...
│                                                                          │
│   ┌───────────────────────────────────────────────┐                      │
│   │  zc-gitops (Git, the source of truth)         │                      │
│   │    - bootstrap/                               │                      │
│   │    - applicationsets/                         │                      │
│   │    - cluster-overrides/                       │                      │
│   └───────────────────────────────────────────────┘                      │
...
│                       ▼                                                  │
│   ┌───────────────────────────────────────────────┐                      │
│   │  ArgoCD on shared-staging-devops              │                      │
│   │  ArgoCD on shared-prod-devops                 │                      │
...
`shared-staging-devops` and `shared-prod-devops` are isolated from workload clusters intentionally:
...
spec:
  project: default
  source:
    repoURL: https://gitlab.com/zencity/zc-gitops.git
...
1. **Author the chart** under `zc-gitops/charts/<addon>/` matching the current Terraform `helm_release` values.
...
By layoff (May 13, 2026), only `karpenter` and the deprecating `nginx-ingress` remained on Terraform-managed Helm. Every other addon is reconciled by ArgoCD ApplicationSets from `zc-gitops`. Karpenter is intentionally last because its `helm_release` is referenced by the EKS module — moving it requires module refactoring rather than a clean addon swap.

## Related

- [GitOps engine project page](/projects/gitops-engine/) — the numbers and where the migration landed
- [Cross-account TGB architecture](/architecture/cross-account-tgb/) — the AWS LBC in workload clusters that this engine deploys
